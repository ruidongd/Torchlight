...
All of this is done in a brute-force algorithm (complete but not optimal), iterating over the whole set of n-sized combinations. We are currently working on optimizing the algorithm by weighting towards the center, first, but the algorithm works fine for grids sized 7 and below.
